	...
	useEffect(() => {
		(async () => {
			try {
				...
				const res: ListResponseNonPaging<AdvertiseBanner> =
					await advertiseApi.getListBanner();
		...
		<NewTopArtContainer>
			<SwiperContainer ref={swiperRef}>
				<SwiperWrapper distance={left}>
					...
					{!isLoading && listBanner && listBanner.length > 0 && (
						<Swiper
							slidesPerView={3}
							spaceBetween={15}
							loop={true}
												...
												>
													...
													>
														<ButtonGradient
															onClick={() =>
																navigate(
																	`${PATH_COLLECTION.detail}/${image.collectionId}`
																)
															}
														>
															View now
