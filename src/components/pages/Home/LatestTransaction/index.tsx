...
//models
import { LatestTransaction as LatestTransactionModel, Response } from 'models';
...
export const LatestTransaction = (props: LatestTransactionProps) => {
	...
	useEffect(() => {
		(async () => {
			setIsLoading(true);
			try {
				const res: Response<any> = await tradingApi.getLatestTransaction();
				setListLatest(res.data);
				setIsLoading(false);
			} catch (error: any) {
