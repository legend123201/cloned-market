...
import MSTToken from 'assets/images/header/logo-token-mst.webp';
//model
...
const PersonalAccount: React.FC = () => {
	...
	useEffect(() => {
		...
		(async () => {
			try {
				if (!isSupportChainId(chainId)) return;
							...
							<Box sx={{ height: '100%', position: 'relative' }}>
								<Stack spacing={3}>
									...
									<ListLink spacing={2}>
										...
										<LinkItem
											href={`/#${PATH_COLLECTION.myCollection}`}
											onClick={(e: any) => {
												e.stopPropagation();
												setOpenDrawer(false);
											}}
