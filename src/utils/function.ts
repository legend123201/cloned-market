...
export const isSupportChainId = (chainId: number): boolean => {
...
export const isPlatformTokenAddress = (address: string, chainId: number) => {
	return isEqualAddress(address, CONTRACT[chainId].FlatformCoin);
};

export const isOurCollection1155Address = (address: string, chainId: number) => {
	return isEqualAddress(address, CONTRACT[chainId].MetaSpacecyAssetShared);
};
