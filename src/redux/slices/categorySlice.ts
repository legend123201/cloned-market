...
export const slice = createSlice({
	...
	reducers: {
		...
		fetchAllCollectionFirstLoadSuccess(state, action: PayloadAction<ListResponse<any>>) {
			state.isLoading = false;
			state.isSuccess = true;
			state.listCollections = action.payload.data;
		},
		fetchAllCollectionSuccess(state, action: PayloadAction<ListResponse<any>>) {
			state.isLoading = false;
			state.isSuccess = true;
		...
		fetchCollectionByIdSuccess(state, action: PayloadAction<any>) {
			state.isLoading = false;
			state.isSuccess = true;
			state.collectionItem = action.payload.data;
		},
...
export const selectListCollection = (state: RootState) => state.category.listCollections;
export const selectCollectionItem = (state: RootState) => state.category.collectionItem;
export const selectListCollectionsByOwnerOrCreatorItems = (state: RootState) =>
	state.category.listCollectionsByOwnerOrCreatorItems;
export const selectPagination = (state: RootState) => state.category.pagination;
export const selectLoading = (state: RootState) => state.category.isLoading;
export const selectFilter = (state: RootState) => state.category.filter;
export const selectHasNextPage = (state: RootState) => state.category.hasNextPage;
